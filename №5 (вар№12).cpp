...
public:
	...
	//ôóíêöèþ âûâîäà ìîæíî áóäåò ïåðåîïðåäåëèòü â ïðîèçâîäíûõ êëàññàõ, êîãäà áóäåò ÿñíà êîíêðåòèêà
	virtual void print()
	{
...
public:
	...
	void GaussianElimination(double** A, int m, int n) {
		...
		for (i = 0; i < m; i++) {
			if (A[i][j] == 0) {
				// Ïîèñê ïåðâîãî íåíóëåâîãî ýëåìåíòà â ñòîëáöå
				for (k = i + 1; k < m; k++) {
					if (A[k][j] != 0) {
						// Ìåíÿåì ìåñòàìè ñòðîêè
						for (int l = 0; l < n; l++) {
							t = A[i][l];
							A[i][l] = A[k][l];
							A[k][l] = t;
				...
				// Åñëè íå íàøëè íåíóëåâîé ýëåìåíò, ïåðåõîäèì ê ñëåäóþùåìó ñòîëáöó
				if (k == m) {
					j++;
					continue;
				}
			}
			// Äåëåíèå ïåðâîé ñòðîêè íà ãëàâíûé ýëåìåíò
			t = A[i][j];
			for (int l = j; l < n; l++) {
				A[i][l] /= t;
			}
			// Âû÷èòàíèå ïåðâîé ñòðîêè èç âñåõ íèæíèõ ñòðîê
			for (k = i + 1; k < m; k++) {
				t = A[k][j];
				for (int l = j; l < n; l++) {
					A[k][l] -= t * A[i][l];
